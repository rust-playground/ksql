...
//!     assert_eq!(Part::Bool(true), result);
//!     Ok(())
//! }
//! ```
//!
...
pub struct Expression {
    // TODO: remove pub from parts
    pub parts: Vec<Part>,
}

/// Represents the calculated Expression result and also the parsed expression.
#[derive(Debug, PartialEq, Clone, Serialize)]
#[serde(untagged)]
pub enum Part {
    Add { lhs: usize, rhs: usize },
    Subtract { lhs: usize, rhs: usize },
    Multiple { lhs: usize, rhs: usize },
    Divide { lhs: usize, rhs: usize },
    Null,
    String(String),
    Number(f64),
    Bool(bool),
    DateTime(DateTime<Utc>), // What to put here arg! do we preserve the original zone etc..?
    Object(BTreeMap<String, Part>),
    Array(Vec<Part>),
}
...
impl<'a> From<gjson::Value<'a>> for Part {
    fn from(v: gjson::Value) -> Self {
        match v.kind() {
            Kind::Null => Part::Null,
            Kind::String => Part::String(v.str().to_string()),
            Kind::Number => Part::Number(v.f64()),
            Kind::False => Part::Bool(false),
            Kind::True => Part::Bool(true),
            Kind::Array => {
...
impl<'a> Parser<'a> {
    fn new(exp: &'a [u8], tokenizer: Tokenizer<'a>, parts: Vec<Part>) -> Self {
        Parser {
            exp,
            tokenizer,
            parts,
    ...
    ///
    /// Will return `Err` the expression is invalid.
    pub fn parse_bytes(expression: &[u8]) -> anyhow::Result<Expression> {
        let tokenizer = Tokenizer::new_bytes(expression);
        let mut parser = Parser::new(expression, tokenizer, Vec::new());
        parser.parse_it()?;

        if parser.parts.is_empty() {
    ...
    // shunting yard algo variant
    fn parse_it(&mut self) -> anyhow::Result<()> {
        let mut output: Vec<Part> = Vec::new();
        let mut stack: Vec<Vec<Part>> = vec![Vec::new()]; // initial stack engaged
        let mut depth = 0;
        // Attempt to parse something simple but with nesting
        // (1 + (1 + 1)) - (1 + 1)

        // [src/parser.rs:1020] &ex = Add {
        //     left: Add {
        //         left: Num {
        //             n: 1.0,
        //         },
            ...
            if let Some(token) = self.tokenizer.next() {
                ...
                match token.kind {
                    ...
                    TokenKind::Add => {
                        ...
                        let rhs = depth * 2;
                        let lhs = rhs - 1;
                        let p = Part::Add { lhs, rhs };

                        let last_stack = stack.last_mut().unwrap();
                        while let Some(v) = last_stack.pop() {
                            output.push(v);
                        }
                    ...
                    TokenKind::CloseParen => {
                        let mut last_stack = stack.pop().unwrap(); // TODO: handle error better
                        while let Some(v) = last_stack.pop() {
                            output.push(v);
                        }
                    }
                ...
                // Cleanup any remaining stack entries
                ...
                // post-process for final representation
                let l = output.len();
                let mut results = VecDeque::with_capacity(output.len());
    ...
    // // close but not quite
    // fn parse_it(&mut self) -> anyhow::Result<()> {
    //     // let mut output: VecDeque<Part> = VecDeque::new();
    //     let mut output: Vec<VecDeque<Part>> = vec![VecDeque::new()];
    //     let mut stack: Vec<Vec<Part>> = vec![Vec::new()]; // initial stack engaged
    //
    //     // Attempt to parse something simple but with nesting
    //     // (1 + (1 + 1)) - (1 + 1)
    ...
    //                             .parse()?,
    //                     );
    //                     output.last_mut().unwrap().push_back(n);
    //                 }
    //                 TokenKind::Add => {
    //                     let lhs = output.len() - 1;
    //                     let rhs = output.last().unwrap().len() + lhs;
    //                     let p = Part::Add { lhs, rhs };
    //
    //                     // let current_stack = stack.last_mut().unwrap();
    //                     //
    //                     // if !current_stack.is_empty() {
    //                     //     let mut out = VecDeque::new();
    //                     //     while let Some(v) = current_stack.pop() {
    //                     //         out.push_back(v);
    //                     //     }
    //                     //     output.push(out);
    //                     // }
    //                     let mut last_stack = stack.pop().unwrap(); // TODO: handle error better
    //                     let last_output = output.last_mut().unwrap();
    //                     while let Some(v) = last_stack.pop() {
    //                         last_output.push_back(v);
    //                     }
    //
    //                     stack.push(Vec::new());
    //                     stack.last_mut().unwrap().push(p);
    //                     // output.last_mut().unwrap().push_front(p);
    //                 }
    //                 TokenKind::OpenParen => {
    //                     stack.push(Vec::new());
    ...
    // making it a two phase parsing process.
    // Process each expression one-by-one, each new expression is another rhs value
    // Then post process all gathered expressions combining onto one array, shifting each
    // Operations indexes by the total length of previously combined expressions.
    // fn parse_it(&mut self) -> anyhow::Result<()> {
    //     // must have an array of expressions to simulate nested operation parsing
    //
    //     let mut expressions: VecDeque<VecDeque<Part>> = VecDeque::new();
    //     let mut lhs: Option<VecDeque<Part>> = None;
    //     let mut rhs: Option<VecDeque<Part>> = None;
    ...
    //                 // if there's already an lhs
    //                 dbg!(operation);
    //                 // parse rhs
    //                 // if rhs is already populated we must be continuing to
    //             } else {
    //                 // if we're still working on the lhs or maybe it's an operation
    //                 dbg!(token);
    //             }
    //
    //             // if let Some(lhs) = lhs {
    //             //     // already has a lhs value
    //             //     if let Some(operation) = operation {
    //             //         // already has a lhs + operation, look for the next rhs
    //             //     } else {
    //             //         // already has a lhs, but no operation yet
    //             //     }
    //             // } else {
    //             //     // parse the first lhs value
    //             //     lhs = Some(self.parse_value(token)?);
    //             // }
    //
    //             // if first {
    //             //     first = false;
    //             //     // look for next value
    //             //     Some(self.parse_value(token)?);
    //             // } else {
    ...
    // fn parse_expression(&mut self) -> anyhow::Result<()> {
    //     let mut first = true;
    ...
    // #[allow(clippy::too_many_lines)]
    // fn parse_value(&mut self, token: Token) -> anyhow::Result<()> {
    //     match token.kind {
    ...
    //
    // #[allow(clippy::too_many_lines)]
    // fn parse_operation(&mut self, token: Token, current: Part) -> anyhow::Result<()> {
    //     match token.kind {
    //         TokenKind::Add => {
    //             let next_token = self.next_operator_token(token)?;
    //             let right = self.parse_value(next_token)?;
    //             let op = Part::Add {
    //                 lhs: self.parts.len() + 1,
    //                 rhs: self.parts.len() + 2,
    //             };
    //             self.parts.push(op);
    //             self.parts.push(current);
...
mod tests {
    ...
    fn test_new() -> anyhow::Result<()> {
        ...
        let expression = r#"1 + (2 + 3) + 4"#;
        // let expression = r#"1 + 2 + 3"#;
